...
export const useGroceryStore = create<GroceryStore>((set, get) => ({
    ...
    addItem: async (input) => {
        ...
        try{
            ...
            set({ error: "Somrthing went wrong" })
        }
    },
    ...
    updateQuantity: async (id, quantity) => {
        ...
        try{
            ...
            const payload = (await res.json()) as ItemResponse;
            if(!res.ok) throw new Error("Error while updating quantity");
            set((state) => ({
                items: state.items.map((item) => (item.id === id ? payload.item : item))
            }))
        }catch(error){
            console.log("Error updating the quantity: ", error);
            set({ error: "Something went wrong" })
        }
    ...
    togglePurchased: async (id) => {
        ...
        try{
            ...
            set({ error: "Somethinh went wrong" })
        }
